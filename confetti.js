...
function getSettings() {
  try {
    return (
      utools.dbStorage.getItem("confetti_settings") || {
        soundEnabled: true,
        confettiEffect: "shootDefault",
      }
    );
  } catch (error) {
    console.error("获取设置失败:", error);
    return { soundEnabled: true, confettiEffect: "shootDefault" }; // 默认开启声音和默认效果
  }
}
...
function shoot() {
  try {
    ...
    // 根据设置选择不同的撒花效果
    let shootPromises;
    const effect = settings.confettiEffect || "shootDefault";
    shootPromises = confettiGun[effect]();
